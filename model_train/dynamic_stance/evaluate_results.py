...
def main():
    ...
    outputs_path = 'output/stance_data/stance_models/output/'

    table_simple_results = {}
    for model in models:
        print(model)
        table_simple_results[model] = []
        path = list(pathlib.Path(outputs_path+model+'/simple_splits').glob('DynamicStance.py_8_0.00001_10_date_date_23-05-2*/test_results_d_stance.txt'))[-1]
        print(path)
        table_simple_results[model].append(get_macro_f1(path, 'data/simple_splits/test.jsonl'))
        exit()

        path_2 = list(pathlib.Path(outputs_path+model+'/raco_augment').glob('DynamicStance.py_8_0.00001_10_date_date_23-05-2*/test_results_d_stance.txt'))[-1]
        print(path_2)
    ...
    for model in models:
        for topic in topics:
            ...
            path = list(pathlib.Path(outputs_path+model+'/topic_splits/'+topic).glob('DynamicStance.py_8_0.00001_10_date_date_23-05-2*/test_results_d_stance.txt'))[-1]
            print(path)
            table_topic_results[(model,topic)].append(get_macro_f1(path, 'data/topic_splits/'+topic+'/test.jsonl'))

            path_2 = list(pathlib.Path(outputs_path+ model + '/raco_augment/topic_splits/' + topic).glob('DynamicStance.py_8_0.00001_10_date_date_23-05-2*/test_results_d_stance.txt'))[-1]
            print(path_2)
